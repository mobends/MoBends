...
public class AssetsModule {
    ...
    public AssetsModule(File configDirectory) {
        ...
        this.localManifestFile = new File(
            modConfigDirectory,
            "asset_manifest.json"
        );
    ...
    private void fetchLocalManifest() {
        ...
        if (this.localManifestFile.isFile()) {
            ...
            try {
                this.localManifest = gson.fromJson(
                    new BufferedReader(new FileReader(this.localManifestFile)),
                    AssetManifest.class
                );
    ...
    private AssetManifest fetchOnlineManifest() {
        ...
        try {
            return sendGetRequest(
                new URL(apiUrl + "/api/asset/manifest"),
                params,
                AssetManifest.class
            );
    ...
    /**
     * Downloads an asset from the external server using SSL certificate bypassing
     * for HTTPS connections to avoid SSL validation issues.
     */
    private void downloadAsset(AssetManifest manifest, AssetDefinition asset)
        throws MalformedAssetException {
        try {
            URL url = new URL(
                manifest.getBaseUrl() + asset.getPath().getAssetPath()
            );
            URLConnection connection = SSLHelper.openConnectionWithSSLBypass(
                url
            );
            ...
            throw new MalformedAssetException(
                String.format("Couldn't download asset: %s", asset.getPath()),
                m
            );
        }
    }
    ...
    public void updateAssets() {
        ...
        Iterable<AssetDefinition> assetsToUpdate =
            AssetManifest.getAssetsToUpdate(localManifest, onlineManifest);
    ...
    public static class Factory implements IModule {
        ...
        public void preInit(FMLPreInitializationEvent event) {
            AssetsModule.INSTANCE = new AssetsModule(
