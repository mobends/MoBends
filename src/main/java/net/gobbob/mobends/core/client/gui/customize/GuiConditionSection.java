...
public class GuiConditionSection
{
	...
	{
		hover = mouseX >= x && mouseX < x + this.getVisualWidth() && mouseY >= y
				&& mouseY < y + GuiAnimationNode.HEIGHT;
	...
	public boolean mouseClicked(int x, int y, int event)
	{
		...
		if (this.selected)
		{
			this.customizeWindow.getParameterEditor().select(this);
		}

		if (this.buttonAddNode.mouseClicked(x, y, event))
		{
			this.addDefaultNode();
			this.customizeWindow.getNodeEditor().updateHeight();
			this.customizeWindow.onNodesChange();
			pressed = true;
		}
	...
	public GuiCustomizeWindow getCustomizeWindow()
	{
		return this.customizeWindow;
	}
	...
	public void removeNode(GuiAnimationNode guiAnimationNode)
	{
		...
		customizeWindow.onNodesChange();;
	}

	public void remove()
	{
		customizeWindow.getNodeEditor().removeSection(this);
	}
