...
public class GuiMBMenu extends GuiScreen{
	...
	public GuiMBMenu(){
		...
		this.buttonPositions = new float[AnimatedEntity.animatedEntities.size()];
	}
	...
	public void initGui()
    {
		...
		if(this.customizeWindow){
			this.buttonList.add(new GuiToggleButton(2,(int)(this.width+this.presetWindowState*-500.0f+10),35+128,AnimatedEntity.animatedEntities.get(this.animatedEntityID).animate).setTitle("Animate", 128-40));
		...
		for(int i = 0;i < AnimatedEntity.animatedEntities.size();i++){
			this.buttonList.add(new GuiButton(100+i, (int) (buttonPositions[i]-80+presetWindowState*-100), 70+i*25, 80, 20, AnimatedEntity.animatedEntities.get(i).displayName));
		}
	...
	protected void keyTyped(char par1, int par2) {
		...
		if(this.customizeWindow){
			...
			else if (this.custom_ModelNameText.isFocused())
	        {
	            ...
	            BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.get(this.custom_currentAction).model = this.custom_ModelNameText.getText();
	        }
    ...
    public void updateScreen() {
        ...
        if(presetWindowState > 0.0f & customizeWindow){
        	...
        	if(mouseY > 25+10+25 & mouseY < 25+10+25+getActionWindowHeight()){
				String varAnim = "";
		    	int displayIndex = 0;
		    	if(BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id) != null){
		    		int i = 0;
		    		for(;i < BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.size();i++){
			    		BendsAction action = BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.get(i);
	...
	protected void mouseClicked(int x, int y, int p_73864_3_)
    {
		if(packsWindow){
			for(int i = 0;i < BendsPack.bendsPacks.size();i++){
				if(x > this.width+this.presetWindowState*-250+10 & x < this.width+this.presetWindowState*-250+10+200 &
				   y > i*70+30 & y < i*70+30+64){
		...
		if(presetWindowState > 0.0f & customizeWindow){
			...
			if(BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id) == null || BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.size() <= 0){
			    		...
			    		if(action != null){
			    			if(x > getActionWindowX()+10 &
			    			   ...
			    			   y < 25+25+10+5+displayIndex*18+16+getYScrollAmount()){
			    				...
			    				this.custom_AnimationNameText.setText(BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.get(this.custom_currentAction).anim);
			    				this.custom_ModelNameText.setText(BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.get(this.custom_currentAction).model);
			    				...
			    				if(x > getActionWindowX()+10+31+64-7+(action.mod != null ? 32-7 : 0)+action.calculations.size()*(64-7) &
				    				...
				    				BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.remove(i);
	...
	protected void actionPerformed(GuiButton par1GuiButton)
    {
        ...
        switch(par1GuiButton.id){
        	...
        	break;
        	case 2:
        		AnimatedEntity.animatedEntities.get(this.animatedEntityID).animate = !AnimatedEntity.animatedEntities.get(this.animatedEntityID).animate;
        	break;
        	case 3:
    ...
    public void drawScreen(int par1, int par2, float par3)
    {
    	...
    	if(presetWindowState > 0.0f & packsWindow){
    		...
    		for(int i = 0;i < BendsPack.bendsPacks.size();i++){
        		...
        		for(int s = 0;s <  BendsPack.bendsPacks.get(i).description.length();s++,lineLength++){
        			text.set(var1,text.get(var1) + BendsPack.bendsPacks.get(i).description.charAt(s));
        			if(this.fontRendererObj.getStringWidth(text.get(var1))*0.5f > 128){
    	    			if(BendsPack.bendsPacks.get(i).description.charAt(s) == ' '){
    	    				lineLength = 0;
    	    				var1++;
    	    				text.add("");
    	    			}
    			...
    			GL11.glPushMatrix();
		    		Minecraft.getMinecraft().renderEngine.bindTexture(ClientProxy.texture_NULL);
		    		GL11.glColor4f(0,0,0,0.5f);
		    		if(i != BendsPack.currentPack) Draw.rectangle(this.width+this.presetWindowState*-250+10,i*70+30,200, 64);
		    		else Draw.rectangle_xgradient(this.width+this.presetWindowState*-250+10,i*70+30,200, 64, new Color(0.0f,0.0f,0.0f,0.5f), new Color(0.1f,1.0f,0.1f,0.5f));
		    		this.drawString(this.fontRendererObj, BendsPack.bendsPacks.get(i).displayName,(int)(this.width+this.presetWindowState*-250+10)+5,i*70+30+5, 0xffffff);
		    		
		    		GL11.glPushMatrix();
		    			GL11.glTranslatef((int)(this.width+this.presetWindowState*-250+10)+5,i*70+30+5+10,0);
		    			GL11.glScalef(0.5f,0.5f,0.5f);
		    			this.drawString(this.fontRendererObj, "By " + BendsPack.bendsPacks.get(i).author, 0, 0, 0x777777);
		    			
		    			for(int s = 0;s < text.size();s++){
			    			this.drawString(this.fontRendererObj, text.get(s),0,20+s*10, 0xffffff);
			    		}
		    		GL11.glPopMatrix();
		    	GL11.glPopMatrix();
    		}
	...
	public void displayCustomizeWindow(){
    	...
    	if(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id == "player")
    		renderLivingEntity((int)(this.width+this.presetWindowState*-500.0f+10+64),150,50, Minecraft.getMinecraft().thePlayer);
		else{
			AnimatedEntity.animatedEntities.get(this.animatedEntityID).entity.worldObj = Minecraft.getMinecraft().theWorld;
			renderLivingEntity((int)(this.width+this.presetWindowState*-500.0f+10+64),150,50, (EntityLivingBase) AnimatedEntity.animatedEntities.get(this.animatedEntityID).entity);
		}
    	...
    	GL11.glMatrixMode(GL11.GL_PROJECTION);
    	GL11.glLoadIdentity();
    	ScaledResolution res = new ScaledResolution(mc, Minecraft.getMinecraft().displayWidth, Minecraft.getMinecraft().displayHeight);
	...
	public void assignAnimationToCurrentAction(String argAnim){
		BendsTarget target = BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id);
		if(target == null)
			return;
	...
	public BendsAction getCurrentAction(){
		...
		if(this.custom_currentAction < BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.size())
			return BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.get(this.custom_currentAction);
		else
			return null;
	}
	...
	public void addNewDefaultAction(String argAnim) {
		if(BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id) == null){
			BendsPack.targets.add(new BendsTarget(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id));
		}
		BendsAction action = new BendsAction(argAnim,"",EnumBoxProperty.ROT,null,0.3f,1);
		BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.add(action);
		this.custom_currentAction = BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.size()-1;
		action.anim = "NULL";
		this.assignAnimationToCurrentAction(argAnim);
		if(this.custom_currentAction > 0){
			action.model = BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.get(this.custom_currentAction-1).model;
			action.prop = BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.get(this.custom_currentAction-1).prop;
			action.axis = BendsPack.getTargetByID(AnimatedEntity.animatedEntities.get(this.animatedEntityID).id).actions.get(this.custom_currentAction-1).axis;
		}
	}
}