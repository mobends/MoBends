...
The four animator files are generated by `animation-lab/tools/gen_animators.ts`
...
`gradle bakePlayer` (`PlayerBake`); the animator is generated by `gen_animators.ts`.
...
* Fist guard, punches, sprint stance, the tool swing curves (dense samples where
  ...
  `gen_animators.ts` from the bits' formulas, with Minecraft's table sine/cosine.
...
* `animation-lab/README.md` explains the lab; `misc/kumo-format.md` documents the format-2
  ...
  `animation-lab/tools/trace_diff.ts` prints per-frame deviations between a golden and an
  animator trace.
...
production (SRG) runtime. `GenVanillaFields.kt` in `src/tools` (`gradle generateVanillaFields`) reads the
