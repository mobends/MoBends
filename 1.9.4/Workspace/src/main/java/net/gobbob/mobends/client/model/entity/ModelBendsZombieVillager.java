...
public class ModelBendsZombieVillager extends ModelZombieVillager{
    ...
    public void setRotationAngles(float argSwingTime, float argSwingAmount, float argArmSway, float argHeadY, float argHeadX, float argNr6, Entity argEntity)
    {
    	...
    	if(data.canBeUpdated()){
			...
			if(data.motion.x == 0.0f & data.motion.z == 0.0f){
				AnimatedEntity.getByEntity(argEntity).get("stand").animate((EntityLivingBase)argEntity, this, data);
				BendsPack.animate(this,"zombie","stand");
			}else{
				AnimatedEntity.getByEntity(argEntity).get("walk").animate((EntityLivingBase)argEntity, this, data);
