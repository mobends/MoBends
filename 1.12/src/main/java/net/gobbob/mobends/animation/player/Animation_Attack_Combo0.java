...
public class Animation_Attack_Combo0 {
	public static void animate(EntityPlayer player,ModelBendsPlayer model,Data_Player data){
		...
		((ModelPart)model.bipedHead).pre_rotation.setSmoothX(-model.bipedBody.rotateAngleX,0.9f);
		((ModelPart)model.bipedHead).pre_rotation.setSmoothY(-model.bipedBody.rotateAngleY-30*handDirMtp,0.9f);
		...
		if(data.motion.x == 0 & data.motion.z == 0){
			...
			((ModelPart) model.bipedBody).rotation.setSmoothY(-70.0f*armSwing*handDirMtp,0.9f);
		}
