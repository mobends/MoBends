    ...
    public void setRotationAngles(float argSwingTime, float argSwingAmount, float argArmSway, float argHeadY, float argHeadX, float argNr6, Entity argEntity)
    {
    	...
    	if(data.canBeUpdated()){
    		...
    		if(((EntityLivingBase)argEntity).isElytraFlying()){
    			aEntity.get("elytra").animate((EntityLivingBase)argEntity, this, data);
    		...
    		if(this.shouldPerformGuardingAnimation((EntityPlayer)argEntity)){
	    		...
	    		if(this.leftArmPose == ArmPose.BOW_AND_ARROW || this.rightArmPose == ArmPose.BOW_AND_ARROW){
	    			...
	    			if((((EntityPlayer)argEntity).getHeldItem(EnumHand.MAIN_HAND) != null && ((EntityPlayer)argEntity).getHeldItem(EnumHand.MAIN_HAND).getItem() instanceof ItemPickaxe) ||
	    				(((EntityPlayer)argEntity).getHeldItem(EnumHand.MAIN_HAND) != null && Block.getBlockFromItem(((EntityPlayer)argEntity).getHeldItem(EnumHand.MAIN_HAND).getItem()) != null)){
    ...
    public void updateModelRenderers(Data_Player data) {
    	for(int i = 0; i < this.boxList.size(); i++) {
			if(this.boxList.get(i) instanceof ModelRendererBends)
				((ModelRendererBends)this.boxList.get(i)).update(data.ticksPerFrame);
		}
