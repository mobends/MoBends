...
public abstract class EntityData {
	...
	public boolean canBeUpdated() {
		return !updatedThisFrame && !(Minecraft.getMinecraft().theWorld.isRemote && Minecraft.getMinecraft().isGamePaused());
	}
	...
	public boolean calcOnGround(){
        ...
        List list = entity.worldObj.getCollisionBoxes(entity.getEntityBoundingBox().addCoord(0,-0.001f,0));
	...
	public boolean calcCollidedHorizontally(){
        ...
        List list = entity.worldObj.getCollisionBoxes(entity.getEntityBoundingBox().addCoord(this.motion.x,0,this.motion.z));
	...
	public void update(float argPartialTicks){
		...
		if(this.ticks > (Minecraft.getMinecraft().thePlayer.ticksExisted+argPartialTicks)) {
			this.onTicksRestart();
			this.ticks = (Minecraft.getMinecraft().thePlayer.ticksExisted+argPartialTicks);
		}
		
		this.ticksPerFrame = Math.max(0.0f, (Minecraft.getMinecraft().thePlayer.ticksExisted+argPartialTicks)-this.ticks);
		this.ticks = (Minecraft.getMinecraft().thePlayer.ticksExisted+argPartialTicks);
	...
	public EnumFacing getLadderFacing() {
		...
		IBlockState block = entity.worldObj.getBlockState(position);
		IBlockState blockBelow = entity.worldObj.getBlockState(position.add(0, -1, 0));
		IBlockState blockBelow2 = entity.worldObj.getBlockState(position.add(0, -2, 0));
	...
	public float getLedgeHeight() {
    	...
    	float clientY = (float) (entity.posY + (entity.posY-entity.prevPosY)*EventHandler_RenderPlayer.partialTicks);
    	
    	BlockPos position = new BlockPos(Math.floor(entity.posX), Math.floor(entity.posY), Math.floor(entity.posZ));
		
		IBlockState block = entity.worldObj.getBlockState(position.add(0, 2, 0));
		IBlockState blockBelow = entity.worldObj.getBlockState(position.add(0, 1, 0));
		IBlockState blockBelow2 = entity.worldObj.getBlockState(position.add(0, 0, 0));
