...
public class Animation_Walk extends Animation{
	...
	public void animate(EntityLivingBase argEntity, IBendsModel argModel, EntityData argData) {
		...
		if(data.isStrafing()){
			((ModelPart)model.bipedRightLeg).rotation.setSmoothX(0.0f, 1.0f);
			((ModelPart)model.bipedLeftLeg).rotation.setSmoothX(0.0f, 1.0f);
			((ModelPart)model.bipedRightLeg).rotation.setSmoothZ(-5.0f+0.5f*(float) ((MathHelper.cos(model.armSwing * 0.6662F) * 1.4F * model.armSwingAmount) / Math.PI * 180.0f),0.2f);
			((ModelPart)model.bipedLeftLeg).rotation.setSmoothZ(-5.0f+0.5f*(float) ((MathHelper.cos(model.armSwing * 0.6662F + (float)Math.PI) * 1.4F * model.armSwingAmount) / Math.PI * 180.0f),0.2f);
		}else{
