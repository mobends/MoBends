...
public class GuiPackEditor {
	...
	public GuiPackEditor() {
		this.titleTextField = new GuiCompactTextField(Minecraft.getMinecraft().fontRendererObj, 120, 14);
	...
	public void display(int mouseX, int mouseY) {
		...
		this.useButton.drawButton(mouseX, mouseY);
		this.editButton.drawButton(mouseX, mouseY);
		this.doneButton.drawButton(mouseX, mouseY);
		...
		if(packList.getSelectedTab().index == GuiPackList.TAB_PUBLIC) {
			...
			this.openFolderButton.drawButton(mouseX, mouseY);
			if(!editMode) {
				if(entry != null) {
