...
public class GuiBendsMenu extends GuiScreen{
	...
	public GuiBendsMenu(){
		...
		this.settingsNodes = new ArrayList<GuiSettingsNode>();
	}
	...
	public void initGui()
    {
		...
		if(this.guiTab == TAB_MAIN) {
			...
			this.buttonList.add(new GuiButton(2,(this.width-80)/2,height/2,80,20,"Settings"));
		...
		if(this.guiTab == TAB_SETTINGS){
			...
			for(SettingsNode node : SettingManager.settings){
				...
				y += guiNode.height+5;
				settingsNodes.add(guiNode);
			}
		}
		
		if(this.guiTab == TAB_CUSTOMIZE){
			if(PackManager.isCurrentPackLocal()) {
				GuiButton applyButton = new GuiButton(4, guiLeft+xSize-50-7, guiTop+77-4, 50, 20,I18n.format("mobends.gui.apply", new Object[0]));
				...
				this.buttonList.add(new GuiButton(5, (width-fontRendererObj.getStringWidth(buttonTitle)-20)/2, guiTop+150, fontRendererObj.getStringWidth(buttonTitle)+20, 20, buttonTitle));
			}
		}
    }
	...
	public void openTab(int tab) {
		...
		switch(this.guiTab) {
			case TAB_CUSTOMIZE:
				...
				this.portraitDisplay.setViewEntity(this.getCurrentAlterEntry().getName().equalsIgnoreCase("player") ? Minecraft.getMinecraft().player : this.getCurrentAlterEntry().getEntity());
	...
	protected void mouseClicked(int x, int y, int p_73864_3_)
    {
		...
		switch(guiTab) {
			case TAB_CUSTOMIZE:
				...
				break;
			case TAB_SETTINGS:
				for(GuiSettingsNode node : settingsNodes) {
					node.mousePressed(Minecraft.getMinecraft(), x, y);
				}
				break;
			case TAB_PACKS:
	...
	public void createANewPack(String string) {
		BendsPack newPack = new BendsPack(null, string, Minecraft.getMinecraft().player.getName(), "A custom pack made by " + Minecraft.getMinecraft().player.getName() + ".");
    ...
    public void drawScreen(int mouseX, int mouseY, float partialTicks)
    {
    	...
    	switch(guiTab) {
    		...
    		case TAB_SETTINGS:
        		...
        		this.drawString(this.fontRendererObj, title, (int) (this.width/2-this.fontRendererObj.getStringWidth(title)/2), 5, 0xffffff);
        		for(GuiSettingsNode node : settingsNodes){
        			node.draw(Minecraft.getMinecraft(), mouseX, mouseY);
        		}
        		break;
    		case TAB_PACKS:
        		this.packEditor.display(mouseX, mouseY);
        		break;
    	}
    	...
    	if(this.popUp != null){
    		...
    		this.drawDefaultBackground();
    		this.popUp.display(mouseX, mouseY);
	...
	public void displayCustomizeWindow(int mouseX, int mouseY, float partialTicks){
		...
		this.drawCenteredString(this.fontRendererObj, I18n.format("mobends.gui.customize", new Object[0]), (int) (this.width/2), this.guiTop+4, 0xffffff);
		
		this.nodeEditor.display(mouseX, mouseY);
		if(!PackManager.isCurrentPackLocal()){
			this.drawCenteredString(fontRendererObj, I18n.format("mobends.gui.chooseapacktoedit", new Object[0]), this.width/2, this.guiTop+135, 0xffffff);
		}
	...
	public void popUpDiscardChanges(int afterAction) {
		this.popUp = new GuiPopUp(I18n.format("mobends.gui.discardchanges", new Object[0]), afterAction, new String[]{"Yes", "No"});
		initGui();
	}
