	...
	public void onPlayerRender(RenderPlayerEvent.Pre event){
		...
		if(AnimatedEntity.getByEntity(event.getEntity()).getAlterEntry(0).isAnimated()){
			...
			if(!currentlyRenderedEntities.contains(event.getEntity().getUniqueID())){
				...
				if(Minecraft.getMinecraft().gameSettings.thirdPersonView == 0 && !EventHandler_RenderPlayer.renderingGuiScreen && player instanceof EntityPlayerSP){
				...
				renderer.doRender(player, event.getX(), event.getY(), event.getZ(), entityYaw, event.getPartialRenderTick());
