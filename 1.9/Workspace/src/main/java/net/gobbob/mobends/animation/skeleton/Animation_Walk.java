...
public class Animation_Walk extends Animation{
	...
	public void animate(EntityLivingBase argEntity, ModelBase argModel, EntityData argData) {
		...
		if(data.isStrafing()){
			((ModelRendererBends)model.bipedRightLeg).rotation.setSmoothX(0.0f, 1.0f);
			((ModelRendererBends)model.bipedLeftLeg).rotation.setSmoothX(0.0f, 1.0f);
			((ModelRendererBends)model.bipedRightLeg).rotation.setSmoothZ(-5.0f+0.5f*(float) ((MathHelper.cos(model.armSwing * 0.6662F) * 1.4F * model.armSwingAmount) / Math.PI * 180.0f),0.2f);
			((ModelRendererBends)model.bipedLeftLeg).rotation.setSmoothZ(-5.0f+0.5f*(float) ((MathHelper.cos(model.armSwing * 0.6662F + (float)Math.PI) * 1.4F * model.armSwingAmount) / Math.PI * 180.0f),0.2f);
		}else{
		...
		((ModelRendererBends)model.bipedHead).rotation.setY(model.headRotationY - ((ModelRendererBends)model.bipedBody).rotation.getNextY(data.ticksPerFrame));
		((ModelRendererBends)model.bipedHead).rotation.setX(model.headRotationX - ((ModelRendererBends)model.bipedBody).rotation.getNextX(data.ticksPerFrame));
